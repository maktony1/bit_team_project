...
<div class="search-again">
	<form action="${pageContext.request.contextPath}/search">
		<input type="text" name="searchWord" placeholder="지역 입력" id="search">
...
<div class="contents">
	...
	<!-- 추후 테이블이 아닌 다른 양식으로 바꿔야함 -->
		<table border="1">
				...
				<!-- 여기를 감싸서 해당 게시물 content_view로 링크를 걸어야함. -->
				<tr>
					<%-- <td>${vo.sImg}</td>
					 --%>
					<td><a href="content_view?board_count=${vo.board_count}">${vo.board_count}</a></td>
					<td class="location">${vo.location.address}</td>
					<td class="lat">${vo.location.lat}</td>
					<td class="lng">${vo.location.lng}</td>
				</tr>
				</c:forEach>
		</table>
	</div>
